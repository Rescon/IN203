...
int main( int nargs, char* vargs[])
{
  ...
  std::cout << "Temps produits scalaires séquentiel: " << elapsed_seconds.count() 
              << std::endl;
...
// ----OpenMP----
  for( int Th_num = 1; Th_num < 9; Th_num++){
    start = std::chrono::system_clock::now();
    double result[nbSamples] = {0};
    #pragma omp parallel for shared(U,V) reduction(+:result) num_threads(Th_num)
      for ( int iSample = 0; iSample < nbSamples; ++iSample )
        result[iSample] = dot(U[iSample],V[iSample]);
    end = std::chrono::system_clock::now();
    elapsed_seconds = end-start;
    ...
    if(sum == sum_OMP){
      std::cout << "Temps produits scalaires avec " << Th_num << " threads: " << elapsed_seconds.count() << std::endl;
    }else{
      std::cout << "Avec " << Th_num << " threads, il y'a erreur de calcul. "  << std::endl;
    }
  }
